function ghost_tgts = analyzeFalseTargets(xtx,xrx,xtgt,vtgt,ft_delays_m, ft_dopp_shifts_mps, rng_std_dev,rr_std_dev,doa_std_dev)
...
%   doa_std_dev Receiver AOA estimation error [default = pi]
%
% OUTPUTS:
...
% Parse inputs
if nargin < 7 || isempty(rng_std_dev)
    rng_std_dev = 1e3; % use 1 km by default
end

if nargin < 8 || isempty(rr_std_dev)
    rr_std_dev = .5; % use .5 m/s by default
end

if nargin < 9 || isempty(doa_std_dev)
...
% Add false target returns to range/range-rate calculations
n_ft = numel(ft_delays_m);
rng_bi = reshape(rng_bi + reshape(ft_delays_m,1,1,1,n_ft),ntx,nrx,ntgt*n_ft);
rr_bi = reshape(rr_bi + reshape(ft_dopp_shifts_mps,1,1,1,n_ft),ntx,nrx,ntgt*n_ft);
% False Targets have the same angle of arrival
psi = reshape(repmat(psi,[1,1,n_ft]),nrx,ntgt*n_ft);
psi_elev = reshape(repmat(psi_elev,[1,1,n_ft]),nrx,ntgt*n_ft);
% Change the target counter
ntgt = ntgt*n_ft;
