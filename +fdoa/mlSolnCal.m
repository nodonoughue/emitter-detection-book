function [x_est,alpha_est, beta_est, A,x_grid] = mlSolnCal(x_fdoa,v_fdoa,zeta,x_cal,zeta_cal,C,C_beta,x_ctr,search_size,epsilon,fdoa_ref_idx)
...
if isempty(search_size) || isscalar(search_size)
    ...
    alpha_srch = 100*ones(n_fdoa,1);
    beta_srch = [25*ones(numel(x_fdoa),1);
...
theta_a = @(alpha) cat(1,x_cal(:),alpha,[x_fdoa(:);v_fdoa(:)]);
ell_a = @(alpha) fdoa.loglikelihoodUnc(x_fdoa, v_fdoa, zeta_cal, C, C_beta, theta_a(alpha), fdoa_ref_idx);
...
ell_b = @(beta) fdoa.loglikelihoodUnc(x_fdoa, v_fdoa, zeta_cal, C, C_beta, theta_b(beta), fdoa_ref_idx);
...
ell = @(x) fdoa.loglikelihoodUnc(x_fdoa, v_fdoa, zeta, C, C_beta, theta_x(x), fdoa_ref_idx);
