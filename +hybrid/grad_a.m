function J = grad_a(x_aoa, x_tdoa, x_fdoa, v_fdoa, x_source,tdoa_ref_idx,fdoa_ref_idx, do2DAoA, alpha_aoa, alpha_tdoa, alpha_fdoa)
% J = grad_a(x_aoa, x_tdoa, x_fdoa, v_fdoa, x_source,tdoa_ref_idx,...
...
if n_aoa > 0
    J_aoa = triang.grad_a(x_aoa, x_source, do2DAoA, alpha_aoa);
else
    J_aoa = [];
end
...
if n_tdoa > 0
    J_tdoa= tdoa.grad_a(x_tdoa, x_source, tdoa_ref_idx, alpha_tdoa);
else
    J_tdoa = [];
end
...
if n_fdoa > 0
    J_fdoa= fdoa.grad_a(x_fdoa, v_fdoa, x_source, fdoa_ref_idx, alpha_fdoa);
else
    J_fdoa = [];
end
...
if n_source <= 1
    ...
    [n_alpha_a,m_aoa,~] = size(J_aoa);
    [n_alpha_t,m_tdoa,~] = size(J_tdoa);
    [n_alpha_f,m_fdoa,~] = size(J_fdoa);

    J_1 = cat(2, J_aoa, zeros(n_alpha_a,m_tdoa+m_fdoa,n_source));
    J_2 = cat(2, zeros(n_alpha_t,m_aoa,n_source), J_tdoa, zeros(n_alpha_t,m_fdoa,n_source));
    J_3 = cat(2,zeros(n_alpha_f,m_aoa+m_tdoa,n_source),J_fdoa);
    J = cat(1,J_1,J_2,J_3);
end