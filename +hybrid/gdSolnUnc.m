function [x,x_full] = gdSolnUnc(x_aoa, x_tdoa, x_fdoa, v_fdoa, z,C,x_init,alpha,beta,epsilon,max_num_iterations,force_full_calc,plot_progress,tdoa_ref_idx,fdoa_ref_idx)
% [x,x_full] = gdSolnUnc(x_aoa, x_tdoa, x_fdoa, v_fdoa, z,C,x_init,alpha,...
...
n_tdoa_msmt = numel(tdoa_test_idx_vec);
n_fdoa_msmt = numel(fdoa_test_idx_vec);

% Initialize measurement error and Jacobian function handles
% theta vector contains x, alpha, and beta.  Let's define the
% indices
x_ind = 1:n_dim;
alpha_a_ind = x_ind(end) + (1:n_aoa);
alpha_t_ind = alpha_a_ind(end) + (1:n_tdoa_msmt);
alpha_f_ind = alpha_t_ind(end) + (1:n_fdoa_msmt);
...
y = @(theta) z - hybrid.measurement(reshape(theta(beta_a_ind),n_dim,n_aoa), ...   % x_aoa
                                    ...
                                    reshape(theta(alpha_t_ind),n_tdoa_msmt,1), ...% alpha_tdoa
                                    reshape(theta(alpha_f_ind),n_fdoa_msmt,1));   % alpha_fdoa
...
th_init = [x_init; zeros(n_aoa+n_tdoa_msmt+n_fdoa_msmt,1); x_aoa(:); x_tdoa(:); x_fdoa(:); v_fdoa(:)];

% Call the generic Least Square solver
[th,th_full] = utils.gdSoln(y,J,C_tilde,th_init,alpha,beta,epsilon,max_num_iterations,force_full_calc,plot_progress);
