function [x_est,alpha_est, beta_est, A,x_grid] = mlSolnCal(x_aoa,x_tdoa,x_fdoa,v_fdoa,zeta,x_cal,zeta_cal,C,C_beta,x_ctr,search_size,epsilon,tdoa_ref_idx,fdoa_ref_idx)
...
if isempty(th_ctr) || isscalar(th_ctr)
    ...
    beta_ctr = [x_aoa(:); x_tdoa(:); x_fdoa(:); v_fdoa(:)];
...
if isempty(search_size) || isscalar(search_size)
    ...
    alpha_srch = [5*ones(m_aoa,1); 100*ones(n_tdoa,1); 100*ones(n_fdoa,1)];
    beta_srch = [25*ones(numel(x_aoa)+numel(x_tdoa)+numel(x_fdoa),1);
                 10*ones(numel(x_fdoa),1)];
...
theta_a = @(alpha) cat(1,x_cal(:),alpha,[x_aoa(:);x_tdoa(:);x_fdoa(:);v_fdoa(:)]);
ell_a = @(alpha) hybrid.loglikelihoodUnc(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta_cal, C, C_beta, theta_a(alpha), tdoa_ref_idx, fdoa_ref_idx);
...
ell_b = @(beta) hybrid.loglikelihoodUnc(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta_cal, C, C_beta, theta_b(beta), tdoa_ref_idx, fdoa_ref_idx);
...
ell = @(x) hybrid.loglikelihoodUnc(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta, C, C_beta, theta_x(x), tdoa_ref_idx, fdoa_ref_idx);
