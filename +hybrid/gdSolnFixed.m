function [x,x_full] = gdSolnFixed(x_aoa, x_tdoa, x_fdoa, v_fdoa, z,C,x_init,a,tol,alpha,beta,epsilon,max_num_iterations,force_full_calc,plot_progress,tdoa_ref_idx,fdoa_ref_idx)
% [x,x_full] = gdSolnFixed(x_aoa, x_tdoa, x_fdoa, v_fdoa, z,C,x_init,a,tol,alpha,...
...
% Parse inputs
if nargin < 17 || ~exist('fdoa_ref_idx','var')
    fdoa_ref_idx = [];
end

if nargin < 16 || ~exist('tdoa_ref_idx','var')
    tdoa_ref_idx = [];
end

if nargin < 15 || ~exist('plot_progress','var')
    plot_progress = false;
end

if nargin < 14 || ~exist('force_full_calc','var')
    force_full_calc = false;
end

if nargin < 13 || ~exist('max_num_iterations','var')
    max_num_iterations = [];
end

if nargin < 12 || ~exist('epsilon','var')
    epsilon = [];
end

if nargin < 11 || ~exist('beta','var')
    beta = [];
end

if nargin < 10 || ~exist('alpha','var')
    alpha = [];
end
