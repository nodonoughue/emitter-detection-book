...
noise_rdoa = L_rdoa*randn(size(L_rdoa,2),2);

C_rrdoa = utils.resampleCovMtx(C_rroa,n_tdoa);
L_rrdoa= chol(C_rrdoa,'lower');
noise_rrdoa = L_rrdoa*randn(size(L_rrdoa,2),2);

noise = cat(1,noise_rdoa,noise_rrdoa);
zeta = z + noise(:,1);
zeta_cal = z_cal + noise(:,2);
...
noise_cal2 = L_rrdoa*randn(size(L_rrdoa,2),1); % Just FDOA noise for cal
zeta_cal2 = z_cal2 + noise_cal2;
...
x_est_cal2 = hybrid.gdSoln([],x_tdoa,x_fdoa,v_fdoa_est,zeta,C_tf,x_init,[],[],[],[],false,false,n_tdoa,n_fdoa);
...
hdl = plot(x_est_full(1,:)/1e3,x_est_full(2,:)/1e3,'--');
utils.excludeFromLegend(hdl);
scatter(x_est(1)/1e3,x_est(2)/1e3,'s','filled','Color',hdl.Color,'DisplayName','Solution (w/o cal)');
hdl = plot(x_est_cal_full(1,:)/1e3,x_est_cal_full(2,:)/1e3,'--');
utils.excludeFromLegend(hdl);
scatter(x_est_cal(1)/1e3,x_est_cal(2)/1e3,'s','filled','Color',hdl.Color,'DisplayName','Solution (w/cal)');
hdl = plot(x_est_cal2(1,:)/1e3,x_est_cal2(2,:)/1e3,'--');
utils.excludeFromLegend(hdl);
scatter(x_est_cal2(1)/1e3,x_est_cal2(2)/1e3,'s','filled','Color',hdl.Color,'DisplayName','Solution (w/FDOA cal)');
grid on
legend('Location','NorthEast');
